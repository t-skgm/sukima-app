...
| **空き期間** | Vacant Period | 予定・ブロックがない連続する休日（土日・祝日）。平日は含まない |
| **連休** | Long Weekend | 3〜5日の空き期間で週末と祝日の両方を含むもの。カレンダー上で強調表示される |
...
- 連続する休日（土日・祝日）で予定・ブロックがない期間を自動検出
- 平日は空き期間として計上しない（連続する休日のみが対象）
- 祝日が土日と隣接する場合は1つの連続休日として結合（例: 土日+月曜祝日=3連休）
- 月をまたぐ連続休日も分割せず1つの空き期間として扱う
...
1. 予定（events）・ブロック期間（blocked_periods）から占有日付Setを構築
2. 範囲内の平日（土日でも祝日でもない日）を占有扱いとして追加
3. 占有されていない連続する日付（=連続する休日）をギャップとして検出
4. 最小日数（デフォルト3日）でフィルタリング
5. 3〜5日で週末と祝日の両方を含む場合は `isLongWeekend: true`

**ポイント:**
- 平日は空き期間に含めず、連続する休日（土日・祝日）のみを抽出
- 祝日が土日と隣接する場合は1つの連続休日として結合される（例: 土日+月曜祝日=3連休）
- 月境界をまたぐ連続休日も分割されない
