...
export async function listEvents(db: Database, familyId: string): Promise<EventOutput[]> {
...
export async function createEvent(
	...
	const result = await db
		...
		.bind(familyId, input.eventType, input.title, input.startDate, input.endDate, input.memo ?? '', now, now)
		.first<{ id: number }>()
...
export async function deleteEvent(db: Database, familyId: string, id: number): Promise<void> {
	const result = await db.prepare('DELETE FROM events WHERE id = ? AND family_id = ?').bind(id, familyId).run()
