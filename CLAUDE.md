...
- **TDD**: t-wada式のテスト駆動開発で実装する（下記参照）

### t-wada式TDD

ドメインロジックの実装・修正には、t-wada（和田卓人）氏が提唱するTDDサイクルに従う。

**基本サイクル（Red → Green → Refactor）:**
1. **Red**: まず失敗するテストを書く（期待する振る舞いを先に定義）
2. **Green**: テストを通す最小限のコードを書く（仮実装・明白な実装）
3. **Refactor**: テストが通る状態を維持しながらコードを改善する

**原則:**
- テストを書く前にプロダクションコードを書かない
- 失敗するテストが1つある状態でのみプロダクションコードを書く
- 各ステップは小さく刻む（一度に大きな変更をしない）
- テストコードもリファクタリング対象（重複除去、意図の明確化）

**実践の流れ:**
```
1. 目的の振る舞いを表現するテストを1つ書く → 実行して失敗を確認（Red）
2. そのテストだけを通す最小のコードを書く → 実行して成功を確認（Green）
3. コードの重複や構造を改善する → テストが通ることを確認（Refactor）
4. 1に戻る
```
